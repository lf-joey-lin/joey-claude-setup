---
name: paperwork
description: File the paperwork for a finished momentum branch - create the TFS work item (story or bug) on Joey's Momentum board from what the branch actually did, then open the matching pull request and link the two together. Runs after wrap-it-up, which leaves the branch merged with origin/main, reviewed, tested, green on the local gate, pushed, and explained in a report. paperwork reads that report, writes the description and the acceptance criteria from the work the branch really contains, creates the item in Active state assigned to Joey via create-tfs, then opens the PR from the repo's own template with the item linked in the body and adds the PR back as a Hyperlink relation on the item's Links tab. Every run creates the item and opens a PR; the PR is a draft unless --pr asks for ready-for-review, and only a ready PR gets the to-be-translated label when the branch changed an en.json, and the dev-bot-laserfiche reviewer. Writes no code, runs no checks, and never creates a second item or a second PR. Invoke when the user types /paperwork, or asks to "do the paperwork", "file the paperwork", "create the item and the PR", "open the PR for this branch", or "wrap up the housekeeping" once wrap-it-up has finished.
---
...
Joey's workflow is `new-work` -> implement -> `wrap-it-up` -> `paperwork`. Every
skill before this one deliberately refuses to touch the board or open a PR, so
this is the only place that does. That matters because both are outward-facing on
shared systems: a wrong story quietly attributes the work to someone else's
scope, and a PR against `main` asks real people for their attention.

**It writes no code and runs no checks.** `wrap-it-up` already merged
`origin/main`, ran the reviews, wrote the tests, and held the `prepare-to-ship`
gate. If any of that is not true, this is the wrong skill (see Preconditions).
...
- **Both halves always happen.** Every run creates the work item and opens a PR.
  There is no item-only mode: a branch that is finished enough to file paperwork
  for is finished enough to have a PR sitting on it.
- **The work item comes first**, so the PR body can carry the link `pr-metadata`
  needs. `--item <id>` is the one way to skip creating one, and then the given item
  is used as-is.
...
- **The tree is clean, and the branch is pushed.** `git status --porcelain` empty,
  ...
  work means the PR would not contain what the item claims. Stop and point at
  `ship-it`; do not commit or push anything yourself.
- **`origin/main` is merged in.** No `main-is-merged` means the branch has not
  seen current `main`, so its green gate answers the wrong question. Stop and
  point at `wrap-it-up`; do not merge here.
...
1. **The `wrap-it-up` report**, `artifacts/wrap-it-up/<branch>-report.md`. This is
   the intended input. Its "What this branch does" and "The changes, in the order
   to read them" sections are exactly the material this skill reformats. Its
   "Still needs your eyes" section is the gotcha line for the PR body.
2. **The branch's other handoffs**: `artifacts/wrap-it-up/<branch>.md` (the state
   file, which carries the gate scorecard and the triaged findings), a
...
3. **The diff itself**, when there is no report:
   ...
   Say in the report that you read the diff because no `wrap-it-up` report was
   there. It is not a stop; the prose is just working from less.
...
**One branch is one piece of work**, per `new-work`. If the branch genuinely
...
one per commit, not one per hunk. The commit split `ship-it` already made is a
good starting point (`git log --oneline origin/main..HEAD`), because it was made
on the same rule, but re-group it where a mechanical sweep and its feature ended
up as separate commits and read as one change.
...
tells: the `wrap-it-up` report describes a fix, there is a test that reproduces a
failure, the branch name says fix, the change is a correction rather than an
addition.
...
- **Description** (`System.Description`): a short paragraph on what the branch
  ...
  `wrap-it-up` report's "Not fixed, on purpose" list is where that comes from.
...
**Otherwise create it** with the `create-tfs` skill, passing the type from step 3,
the description and acceptance criteria from step 4, and this:
...
For **`--item <id>`** it is the real check. Line the step 2 bullets up against the
item's acceptance criteria one to one:
...
- **Description of changes**: the step 2 bullets. Short, factual, skimmable. Under
  ten lines of body even for a big change. Then the gotcha, if the `wrap-it-up`
  report surfaced one a reviewer would not guess from the diff.
...
- **the work item**: id, type, title, state, sprint, and whether it was created or
  came from `--item`. The clickable board link.
- **what you guessed**, so Joey can fix it in one pass: the type inference if it
  ...
  one component.
- **where the prose came from**: the `wrap-it-up` report, or the diff because there
  was no report.
...
- **what CI owns that nothing local covered**: the container builds, the migration
  apply, `pr-metadata`, and the contract breaking-change check. `wrap-it-up`'s
  green scorecard is not a promise that CI is green.

## Autonomous mode (headless, under another orchestrator)

See [`../shared/autonomous-pipeline.md`](../shared/autonomous-pipeline.md). Both
of this skill's questions get a default:
...
- **No code, no checks, no git writes to content.** This skill creates a board
  item and a PR and nothing else. No commits, no pushes, no merges, no edits to
  source, tests, or translation catalogs. If the branch is not ready, that is a
  stop pointing at `ship-it` or `wrap-it-up`, never a fix here.
- **One item and one PR per branch, ever.** Check for an existing PR before
  creating one; check with `search_workitem` before retrying a failed creation.
- **The item is created before the PR**, so the body can carry the link
...
- **Never rewrite a pre-existing item's description or acceptance criteria** to
  make the branch look finished. Report the gap and let a human decide.
