---
name: ship-it
description: Commit the current working-tree changes and push them, splitting the work into one commit per logical piece and leaving local-only files uncommitted. Reads the actual diff, groups it, writes short plain-language commit subjects in the repo's own style, commits each group with explicit paths, then pushes the branch. Commit and push only: it opens no pull request, touches no TFS board, merges nothing and runs no checks, so the branch is left exactly where the human works from. The work item and the PR are the `paperwork` skill's job. Never commits to main, never stashes or deletes what it skips. Invoke when the user types /ship-it, or asks to "ship it", "commit and push", "commit my changes", "push what I have", or "get this committed".
---
...
push the branch. That is the whole scope.
...
Out of scope, deliberately, and each owned by another skill:

| Not here | Whose job |
|---|---|
| Amending, rebasing, force-pushing, any history rewrite | nobody's; this repo does not do it |
| Merging `origin/main` into the branch | `wrap-it-up` |
| Lint, tests, coverage, build, a11y | `prepare-to-ship` |
| The TFS work item and the pull request | `paperwork` |

The last row is the one to remember. `ship-it` used to carry a `--pr` mode; it
does not any more. Joey's flow is `new-work` -> implement -> `wrap-it-up` ->
`paperwork`, and `wrap-it-up` calls this skill as its commit-and-push phase.

## Invocation

```
/ship-it [hint]              # commit and push
/ship-it --no-push [hint]    # commit only, leave the branch local
```
...
If the invocation carries `--pr` or `--draft-pr`, those flags are gone. Commit and
push as normal, then say in the report that the PR and the work item are
`/paperwork`, and that it wants the branch merged with `origin/main` and green on
the gate first, which is `/wrap-it-up`. Do not open a PR here.
...
- **Never commit to `main`.** If the branch is `main` (or HEAD is detached), stop
  and say so. Offer to run `new-work` to cut a branch first. Do not create one
  silently - the branch name is the user's call.
...
- **`fr.json`, `es.json`, `en-XA.json`, or the XLIFF memory modified by hand** -
  ...
  edited by a person. A file an `origin/main` merge brought in is not a hand
  edit - check the log before raising this.
...
- **Test and lint edits from the gate are a piece by the same rule.** Tests
  `prepare-to-ship` or `update-tests` wrote go with the code they cover; a pure
  lint autofix sweep is its own commit.
...
This grouping is also what `paperwork` reads back off the log to build the PR's
bullet list, so a split that reflects real decisions saves work later.
...
- `--no-push` skips this step. Say so in the report so nobody assumes the branch
  is on the remote.
...
- if any `en.json` changed: say it now so it is not a surprise later - whatever PR
  this branch ends up on needs the **`to-be-translated`** label before
  `pr-i18n-parity` can pass. `paperwork --pr` adds it when it opens a
  ready-for-review PR; on a draft, or if the PR is opened by hand, it is Joey's to
  add (`gh pr edit <n> --add-label to-be-translated`). Never try to fix parity
  locally, and never report `pr-i18n-parity` as a failure.
- if a commit carries a mixed file from step 4, say which one and what rode along

When this skill was invoked directly rather than as a `wrap-it-up` phase, end with
one line naming what has not happened yet: nothing was merged, no checks were run,
and there is no work item or PR. Point at `/wrap-it-up` if the branch has not had
its review-and-gate pass, `/paperwork` if it has.
...
|---|---|
| Step 3 ambiguous file question | commit only what is unambiguously real work; leave anything ambiguous in the tree and list it |
| Step 3 debug-leftover question | commit the file as it is and name the leftover in the report; do not strip it unattended |

That is the whole list, because there is only one gate left. The hard stops do not
soften: a secret, a hand-edited translation catalog, and the branch being `main`
all still stop the run, and a stop is reported rather than worked around.
...
- No amending, no rebasing, no force-pushing, no history rewriting. This skill only
  adds commits on top.
- **It merges nothing, opens nothing, runs nothing and touches no board.** There is
  no flag that changes that. An orchestrator calling ship-it as a phase gets
  exactly commits and a push, which is what `wrap-it-up` relies on.
